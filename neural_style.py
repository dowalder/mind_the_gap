...
def main():
    ...
    if args.subcommand == "train":
        ...
        myfile = pathlib.Path(args.content_image)
        if myfile.is_dir():
            output = pathlib.Path(args.output_image)
            for f in myfile.glob("*.jpg"):
                args.content_image = f.as_posix()
                args.output_image = (output / f.name).as_posix()
                stylize(args)
        else:
            stylize(args)
